#!/usr/bin/env python3
"""
Combined GrovePi fire monitor for Raspberry Pi.

This script supports:
- local sensor monitoring and LED/buzzer alerts
- MQTT publishing of sensor readings
- MQTT command handling (buzzer_on, buzzer_off, all_off, set_lights)
- MQTT config updates for thresholds

Use environment variables to enable MQTT:
  MQTT_BROKER=<broker_ip> MQTT_PORT=1883 DEVICE_CODE=MASTER_ROOM python3 main.py

If MQTT_BROKER is not set, the script will run in local-only mode.
"""
...
TOPIC_PUBLISH_SENSORS = f"fire-detection/{DEVICE_CODE}/sensors"
TOPIC_SUBSCRIBE_COMMANDS = f"fire-detection/{DEVICE_CODE}/commands"
TOPIC_SUBSCRIBE_CONFIG = f"fire-detection/{DEVICE_CODE}/config"
...
def read_all_sensors():
    ...
    try:
        # Set 10-second timeout for sensor reads
        ...
        print("[SENSOR ERROR] Read timeout - GrovePi may be unresponsive")
        signal.alarm(0)
...
def on_connect(client_obj, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        client_obj.subscribe(TOPIC_SUBSCRIBE_COMMANDS)
        client_obj.subscribe(TOPIC_SUBSCRIBE_CONFIG)
...
def on_publish(client_obj, userdata, mid):
...
def setup_mqtt_client():
    if mqtt is None:
        raise RuntimeError("paho-mqtt is not installed. Install it with: pip3 install paho-mqtt")
    client_obj = mqtt.Client(client_id=f"fire-detection-{DEVICE_CODE}")
    if MQTT_USERNAME and MQTT_PASSWORD:
        client_obj.username_pw_set(MQTT_USERNAME, MQTT_PASSWORD)
        print(f"[MQTT] Using credentials: {MQTT_USERNAME}:***")
    client_obj.on_connect = on_connect
    client_obj.on_disconnect = on_disconnect
    client_obj.on_message = on_message
    client_obj.on_publish = on_publish
...
def main():
    ...
    if MQTT_BROKER:
        ...
        print("MQTT disabled: run with MQTT_BROKER=<ip> to enable")
    print("=" * 50 + "\n")
    setup_ports()
    ...
    try:
        while True:
            print("[DEBUG] Starting sensor read...")
            sensors = read_all_sensors()
            print("[DEBUG] Sensor read complete!")
